...
dfall <- read.csv("DirectoryData/SHDirectory.csv", skip = 1, stringsAsFactors = FALSE)
...
for(j in 1:length(classes)){
  ...
  for(k in 1:length(secClasses)){
    ...
    writeLines("\\begin{longtable}{p{100pt}|p{90pt}|p{120pt}|p{60pt}|p{64pt}|p{120pt}|}")
    ...
    for(i in 1:dim(tmpdf)[1]){
      if(tmpdf$Child[i] != ""){
        writeLines(paste0("\\multirow{2}{70pt}{", tmpdf$Child.clean[i], "} & ", tmpdf$Parent1[i], " & ", tmpdf$Parent1.email.clean[i], " & ", tmpdf$Parent1.cell[i], " & ", tmpdf$Parent1.home[i], " & ", "\\multirow{2}{100pt}{", tmpdf$Address[i], "} \\\\"))
